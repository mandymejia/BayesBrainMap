...
#'
#' @param prior_mean (\eqn{V \times Q} matrix) mean maps for each network in the prior,
...
#'
#' @return  A list: theta (list of final parameter estimates), subjNet_mean
...
EM_BBM.spatial <- function(
  ...
  if(!all.equal(dim(prior_var), dim(prior_mean))) {
    stop('The dimensions of prior_mean and prior_var must match.')
  }
  ...
  if(nrow(prior_mean) != nvox) stop('Templates and BOLD must have the same number of data locations (columns).')

  Q <- ncol(prior_mean) #number of networks
  if(Q > nvox) stop('Cannot estimate more networks than data locations.')
  if(Q > ntime) stop('Cannot estimate more networks than time points.')

  prior_var[prior_var < 1e-6] <- 1e-6 #to prevent problems when inverting covariance

  #pre-compute s0, D and D^{-1}*s0
  s0_vec <- as.vector(prior_mean) #grouped by network
  D_vec <- as.vector(sqrt(prior_var)) #grouped by network
  ...
  list(
    subjNet_mean=mu_cov_s$mu_s,
    subjNet_cov=mu_cov_s$cov_s,
...
EM_BBM.independent <- function(
  ...
  if(nrow(prior_mean) != nvox) stop('Templates and BOLD must have the same number of brain locations (columns).')

  Q <- ncol(prior_mean) #number of networks
  if(Q > nvox) stop('Cannot estimate more networks than brain locations.')
  if(Q > ntime) stop('Cannot estimate more networks than time points.')
  ...
  #
  #
  #   LL_vals[iter] <- compute_LL_std(theta, prior_mean, prior_var, C_diag, BOLD)
  #   if (iter > 1) {
...
#' @param prior_mean,prior_var The prior
...
#' @keywords internal
#'
compute_LL_std <- function(theta, prior_mean, prior_var, C_diag, BOLD, verbose){

  # Define the variables -------------------------------------------------------
  nu0_sq <- theta$nu0_sq # length-1 scalar
  A_i1 <- theta$A # nL x nL
  S_0 <- t(prior_mean) # nL x nV
  nu <- prior_var # nV x nL
  C_inv <- diag(1/C_diag)  # (nL x nL) or (nT x nT)
  Y_i <- t(BOLD) # (nL x nV) or (nT x nV)
  ...
  # }, 0))
  # Mandy's version ( Use Trace instead of summing over V, and use the trick that Tr(AB) = Tr(BA) where AB is nV x nV and BA is nL x nL )
...
#' @param H,Hinv For dimension reduction
#' @param s0_vec Vectorized prior means
#' @param D Sparse diagonal matrix of prior standard deviations
#' @param Dinv_s0 The inverse of D times s0_vec
# @param common_smoothness If \code{TRUE}. use the common smoothness version
# of the spatial Bayesian brain map model, which assumes that all networks have the same
...
UpdateTheta_BBM <- function(
  ...
  #     s0_vec = "s_0" (long vector organized by networks)
  #     y (long vector organized by locations)
  #     D, big diagonal matrix of prior standard deviations
  ...
  # nu0sq_hat <- 1/(nL*nV)*(nu0sq_part1 - 2*nu0sq_part2 + nu0sq_part3)
...
#' @param P Permutation matrix for regrouping by locations (instead of by networks.)
...
#'
#' @param Q The number of networks
#' @param nvox The number of spatial locations (\eqn{V})
#'
#' @details If s=(s1,...,sQ) is grouped by networks 1,...Q, then Ps=(s(1),...,s(nvox)) is grouped by locations 1,...,nvox
#'
...
UpdateThetaSQUAREM_BBM <- function(
  ...
  #spatial prior or standard prior?
...
#'
LL2_kappa <- function(
  kappa, Amat, Fmat, Gmat, GFinvG, OplusW, u, v, C1 = 1/(4*pi), Q=NULL){
